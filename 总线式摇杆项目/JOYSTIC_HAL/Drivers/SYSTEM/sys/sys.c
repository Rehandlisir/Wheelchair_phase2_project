 ...
 *
 *              外部晶振为 12M的时候, 推荐值: plln = 28, pllm = 1, pllp = 2, pllq = 2.
 *              得到:Fvco = 8 * (336 / 8) = 336Mhz
 *                   Fsys = pll_p_ck = 336 / 2 = 168Mhz
 *                   Fq   = pll_q_ck = 336 / 2 = 168Mhz
 *
 *              F407默认需要配置的频率如下:
 *              CPU频率(HCLK) = pll_p_ck = 168Mhz
 *              AHB1/2/3(rcc_hclk1/2/3) = 168Mhz
 *              APB1(rcc_pclk1) = pll_p_ck / 1 = 168Mhz
 *              APB1(rcc_pclk2) = pll_p_ck / 1 = 168Mhz
 *
...
uint8_t sys_stm32_clock_init(void)  
{
    ...
    rcc_osc_init.PLL.PLLQ = RCC_PLLP_DIV2;
		rcc_osc_init.PLL.PLLR = RCC_PLLP_DIV2;
    ...
    rcc_clk_init.APB1CLKDivider = RCC_HCLK_DIV1;                 /* APB1分频系数为4 */
    rcc_clk_init.APB2CLKDivider = RCC_HCLK_DIV1;                 /* APB2分频系数为2 */
    ret = HAL_RCC_ClockConfig(&rcc_clk_init, FLASH_LATENCY_4);   /* 同时设置FLASH延时周期为5WS，也就是6个CPU周期 */
    if(ret != HAL_OK)
    {
        return 1;                                                /* 时钟初始化失败 */
    }
