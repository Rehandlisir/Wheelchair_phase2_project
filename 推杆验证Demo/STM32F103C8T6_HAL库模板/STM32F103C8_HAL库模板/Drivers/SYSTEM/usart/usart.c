...
void usart_init(uint32_t baudrate)
{
    ...
    HAL_UART_Receive_IT(&g_uart1_handle, (uint8_t *)uart_rx_data, 1); 
}
	  ...
	  if(huart->Instance == USART1) 
		{
			...
			actor_cmd=uart_rx_data[0] ;
			
			
			HAL_UART_Receive_IT(&g_uart1_handle, uart_rx_data, 1); // 重新启用接收 
