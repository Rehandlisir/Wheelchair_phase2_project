 ...
 * @LastEditTime : 2024-11-05 17:26:23
...
void RightMoterMove(double duty_cycle,uint8_t isrmoter_revers)
{
 ...
 {    
    if (duty_cycle>0)
    {
        atim_tim8_cplm_pwm_set(0,(uint16_t)(duty_cycle*100));// 向前
    }
    else if (duty_cycle<0)
    {
        atim_tim8_cplm_pwm_set((uint16_t)(-duty_cycle*100),0);//向后
    }
    else
    {
        RightMoterStop(); 
 ...
 else
 {
    if (duty_cycle>0)
    {
        atim_tim8_cplm_pwm_set((uint16_t)(duty_cycle*100),0);// 向前
    }
    else if (duty_cycle<0)
    {
        atim_tim8_cplm_pwm_set(0,(uint16_t)(-duty_cycle*100));//向后
    }
    else
    {
        RightMoterStop(); 
