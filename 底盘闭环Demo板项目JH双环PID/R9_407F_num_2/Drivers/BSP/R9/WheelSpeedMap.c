 ...
 * @LastEditTime : 2024-12-18 10:24:45
...
void velocity_maping(VELOCITY_PIn velPlanIn)
{
  ...
  /*速度模长*/
	...
	/***********************************电流环调试程序******************************************************** */
    // Struc_ActuPra_Out.LN_Velocity = Value_limitf(-25.0,Struc_ActuPra_Out.LN_Velocity,25.0);
	// Struc_ActuPra_Out.RN_Velocity = Value_limitf(-25.0,Struc_ActuPra_Out.RN_Velocity,25.0); // 速度约束
    // /********************左右轮目标电流 PID 给定***********************************************/
	// gl_current_pid.SetPoint = Struc_ActuPra_Out.LN_Velocity/2.0;
	// gr_current_pid.SetPoint = Struc_ActuPra_Out.RN_Velocity/2.0;   // 目标电流 0~ 5A
	// // 驱动执行
	// if (gl_motor_data.pidspeed_out > 0.2)
	// {
	// 	LeftMoterMove(fabs(gl_motor_data.pwm), 0); // 前进
	// }
	// else if (gl_motor_data.pidspeed_out < -0.2)
	// {
	// 	LeftMoterMove(-fabs(gl_motor_data.pwm), 0); // 后退
	// }
	// else
	// {
	// 	LeftMoterMove(0, 0);
	...
	// if (gr_current_pid.SetPoint > 0.2)
	// {
	// 	RightMoterMove(fabs(gr_motor_data.pwm), 1);
	// }
	// else if ((gr_current_pid.SetPoint < -0.2))
	// {
	// 	gr_current_pid.SetPoint = -gr_current_pid.SetPoint;
	// 	RightMoterMove(-fabs(gr_motor_data.pwm), 1);
	// }
	// else
	// {
...
{ 
	...
	Struc_ActuPra_Int.set_Maximum_Strspeed =3.0 ;
...
void underpanExcute(void)
{
	...
	Struc_ActuPra_Int.adcy = local_slopelimity(Struc_ActuPra_Int.adcy,25,50);
