...
void vInMeasurementNormal(void) 
{
   ...
   if(ucCheckReadMlxStatusErro(mlxdata.statusByte, MlxInNoneReg_Mode))
    ...
    // printf(mlxdata.usMlx90393StatusErroTimes);
...
*/
    ...
    /*摇杆有效数据段滤波*/
    mlxdata.xdata  =filterValueX_int32(&filter_ADCX, mlxdata.xdata );
    mlxdata.ydata  =filterValueY_int32(&filter_ADCY, mlxdata.ydata);
    // printf("mlxdata.xdata:%d,mlxdata.ydata:%d\n",mlxdata.xdata, mlxdata.ydata);
    delay_ms(2);
...
#ifdef MachineNo_2
    if (mlxdata.xdata ==-1 && mlxdata.ydata==-1) // 摇杆断开
    {
        mlxdata.xdata =15900;
        mlxdata.ydata =15600;
    }
    
    mlxdata.xdata =(15900 - mlxdata.xdata); 
    mlxdata.ydata =(15600 - mlxdata.ydata);
    /*摇杆数据有效段截取*/
    mlxdata.xdata = Value_limit(MIN_XDATA, Value_ResetXzero(-XADC_DIM,  mlxdata.xdata, XADC_DIM), MAX_XDATA);
    mlxdata.ydata = Value_limit(MIN_YDATA, Value_ResetYzero(-YADC_DIM,  mlxdata.ydata, YADC_DIM), MAX_YDATA);
    /*摇杆有效数据段均值滤波滤波*/
