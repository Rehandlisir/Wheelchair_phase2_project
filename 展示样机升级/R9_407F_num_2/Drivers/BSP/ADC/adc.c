 ...
 * note ：
 * 针对 R9系统的所有ADC 数据采集 ，
 * 一、   ADC1 数据采集  包含 11 通道
 * (1) PA0 电机2电流1
 * (2) PA1 电机2电流2
 * (3) PA2 PA3 PA4   ADC_MID ADCX ADCY 
   (4) PC2 PC3 左路电机电流检测
   (5) PA6 PB1    离合器1/2状态检测
	 (6) PC4        电池电压
  （7）PC5     充电状态检测
...
void adc1_nch_dma_init(uint32_t mar)
{
    ...
    adc_channel_set(&g_adc1_nch_dma_handle, ADC_CHANNEL_3,  4, ADC_SAMPLETIME_480CYCLES);
		adc_channel_set(&g_adc1_nch_dma_handle, ADC_CHANNEL_4,  5, ADC_SAMPLETIME_480CYCLES);
    adc_channel_set(&g_adc1_nch_dma_handle, ADC_CHANNEL_6, 6, ADC_SAMPLETIME_480CYCLES);
    adc_channel_set(&g_adc1_nch_dma_handle, ADC_CHANNEL_9, 7, ADC_SAMPLETIME_480CYCLES);
		adc_channel_set(&g_adc1_nch_dma_handle, ADC_CHANNEL_12, 8, ADC_SAMPLETIME_480CYCLES);
		adc_channel_set(&g_adc1_nch_dma_handle, ADC_CHANNEL_13, 9, ADC_SAMPLETIME_480CYCLES);
    adc_channel_set(&g_adc1_nch_dma_handle, ADC_CHANNEL_14, 10, ADC_SAMPLETIME_480CYCLES);
		adc_channel_set(&g_adc1_nch_dma_handle, ADC_CHANNEL_15, 11, ADC_SAMPLETIME_480CYCLES);
...
void adc1_nch_dma_gpio_init(void)
{
    ...
    /* AD采集引脚模式设置,模拟输入 */
    gpioA_init_struct.Pin = GPIO_PIN_0 | GPIO_PIN_1| GPIO_PIN_2 | GPIO_PIN_3 |GPIO_PIN_4 | GPIO_PIN_6;
    ...
    __HAL_RCC_GPIOB_CLK_ENABLE(); /* 开启GPIOB引脚时钟 */
    /* AD采集引脚模式设置,模拟输入 */
    gpioB_init_struct.Pin =  GPIO_PIN_1;
    gpioB_init_struct.Mode = GPIO_MODE_ANALOG;
